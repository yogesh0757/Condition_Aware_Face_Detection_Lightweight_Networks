...
if __name__ == '__main__':
    ...
    fw = open(os.path.join(args.save_folder, 'MAFA_dets.txt'), 'w')

    # testing dataset
    testset_folder = os.path.join('data', args.dataset, 'test-images/images/')
    testset_list = os.path.join('data', args.dataset, 'MAFA-Label-Test/LabelTestAll.mat')

    img_list = loadmat(testset_list)
    test_dataset = []
    for i in range(img_list['LabelTest'].shape[1]):
        test_dataset.append(img_list['LabelTest'][0,i][0][0])
